...
{
    if (ShouldScrollRight(snake, stage)) return Point2D::MakePoint2D(1, 0);
    if (ShouldScrollLeft(snake, stage))  return Point2D::MakePoint2D(-1, 0);
    if (ShouldScrollDown(snake, stage)) return Point2D::MakePoint2D(0, 1);
    if (ShouldScrollUp(snake, stage)) return Point2D::MakePoint2D(0, -1);

    return Point2D::MakePoint2D(0, 0);
}
...
void Renderer::RenderStageSection(Stage &stage)
{
    for (size_t i = 0; i < playfieldHeight; i++)
        ...
        {
            Point2D screenPosition = Point2D::MakePoint2D(j, i);
...
" ______         __         __   __               _______               __            \n\
 ...
 |______/|_____||_____|    |____|__|__|_____|    |_______||__|__|___._||__|__|_____| ver 0.3");
    ...
    mvprintw(GetScreenSize().height/2 + windowHeight/2, GetScreenSize().width/2 - 9, "Press F2 to exit");
...
void Renderer::ScrollScreenElements(Snake& snake, Stage& stage)
{
    ...
    snake.PlaceBodyAtPosition(Point2D::MakePoint2D(snake.GetHeadLocalPosition().x + scrollAmount.x, snake.GetHeadLocalPosition().y + scrollAmount.y));
}
...
void Renderer::RenderSnake(Snake& snake)
{
    ...
    {
        Point2D snakePosition = Point2D::MakePoint2D(snake.body[i].x, snake.body[i].y);
        if (i%2)
...
{
    wattron(window, COLOR_PAIR(3));
    mvwprintw(window, 1, 1," GAME OVER!\t Press F3 to restart");
    wattroff(window, COLOR_PAIR(3));
    RefreshScreen();
}
