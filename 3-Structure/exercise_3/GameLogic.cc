...
{
    // The map size is calculated based on the size of the bounding box passed down to "GenerateMap"
    // Width = point2.x - point1.x
    // Height = point2.y - point1.y
    // Note point 1 is the top-left corner of the bounding box and point 2 is the bottom-right corner.
    BoundingBox box {0, 0, static_cast<int>(stageSize.width), static_cast<int>(stageSize.height)};
    currentMap = mapGenerator.GenerateMap(box);
...
{
    initialPosition = Point2D::MakePoint2D(0,0);
